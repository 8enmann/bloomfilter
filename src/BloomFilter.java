...
public class BloomFilter {
	 ...
	 * 
	 * Based expectedItems and falsePositiveRate, determines the optimal capacity and number of hashes
	...
	/**
	 * True if this key may have been added. Wrong flasePositiveRate fraction of the time.
	 * False if it was definitely not added.
	 * @param key
	 * @return
	 */
	...
	public void add(String key) {
		...
		int hashes[] = getHashes(key);
		for (int hash : hashes) {
			store.setBit(hash % capacity, true);
		}
	}
	 ...
	 * 
	 * This could be made more efficient by using different subsets of each hash multiple times
	 * but it could be premature optimization assuming the keys are reasonably small.
	...
	private int[] getHashes(String key) {
		...
		for (int i = 0; i < this.numHashes; i++) {
			...
			hashes[i] = bytes[0] << 24 | (bytes[1] & 0xFF) << 16 | (bytes[2] & 0xFF) << 8 | (bytes[3] & 0xFF);
